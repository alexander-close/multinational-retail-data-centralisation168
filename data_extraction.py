import pandas as pd
import tabula
import requests
import boto3
import yaml
from io import BytesIO
from database_utils import DatabaseConnector

with open('constants.yaml','r') as f:
  consts = yaml.safe_load(f)
PDF_URL = consts['PDF_URL']
ENDPOINT = consts['ENDPOINT']
# NOS_URL = consts['STORE_URL'].format('number_stores')
# STORE_URL = consts['STORE_URL'].format('store_details/{store_number}')
with open('keys.yaml','r') as g:
  keys = yaml.safe_load(g)
headers = {'x-api-key':keys['x-api-key']}

class DataExtractor:
  def __init__(self):
    self.instance = DatabaseConnector()
    self.number_stores = 0

  def read_rds_table(self,table_name:str):
    engine = self.instance.init_db_engine()
    if table_name in self.instance.list_db_tables():
      return pd.read_sql_table(table_name,engine)
    else:
      return 'Table not in schema.'
    
  def retrieve_pdf_data(self,url:str=PDF_URL):
    df_list = tabula.read_pdf(url, pages='all')
    df = pd.DataFrame()
    for dfs in df_list:
      df = pd.concat([df,dfs])
    return df
  
  def list_number_of_stores(self,headers:dict,endpoint=ENDPOINT):
    endpoint = endpoint.format('number_stores')
    NOS = requests.get(endpoint,headers=headers)
    if NOS.status_code < 400:
      self.number_stores = NOS.json()['number_stores']
      self.NOS_endpoint = endpoint
      return NOS.json()['number_stores']
    else:
      return print(f'Code {NOS.status_code}. Try self.number_stores')
    
  def retrieve_stores_data(self,headers:dict=headers,endpoint=ENDPOINT):
    df = pd.DataFrame()
    NOS = self.list_number_of_stores(headers,endpoint)
    if type(NOS) == int:
      for i in range(NOS):
        RAS = requests.get(endpoint.format(f'store_details/{i}'),headers=headers).json()
        df = pd.concat([df,pd.DataFrame([RAS])])
      return df
    else:
      print("NOS not an int")

  def extract_from_s3(self,url):
    bucket,file = url.replace('s3://','').split('/')[0],url.replace('s3://','').split('/')[1:]
    s3 = boto3.client('s3')
    response = s3.get_object(Bucket=bucket, Key='/'.join(file))
    data = response['Body'].read()
    return pd.read_csv(BytesIO(data))

if __name__ == '__main__':
  test = DataExtractor()
  df = test.read_rds_table('dim_card_details')
  df2=df.copy()
  print(df.head())
  # print(type(df2.join_date[0]))
  print(df.info())
  # print(len(df))
  # print(df.head(5))
  print(df.columns)
#   print(df)